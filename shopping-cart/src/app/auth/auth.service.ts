...
export class AuthService {
    ...
    signup(email: string, password: string) {
        return this.http.post<AuthResponseData>(
            'https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyBl6ALxGHZ55zwr5NGqXkbgAOzCug-StP8',
    ...
    login(email: string, password: string) {
        return this.http.post<AuthResponseData>(
            'https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyBl6ALxGHZ55zwr5NGqXkbgAOzCug-StP8',
            { email, password, returnSecureToken: true }
        )
        .pipe(
            catchError(this.handleError),    //automatically passed response
